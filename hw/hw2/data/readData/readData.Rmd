...
fExtract <- function(lraw, checkin) {
	...
	if (length(ul) > length(l)){
		...
		if(checkin) {
			...
			l.total <- ncol(l.logic) - rowSums(l.logic)
			use <- l.logic
		}
...
readJSON <- function(dirPath, fileName, parallel=FALSE, checkin=FALSE) {
	...
	data.processed <- llply(data.df, fExtract, checkin=checkin, .parallel=parallel)
...
trainingData <- lapply(trainingFiles, function(x) {
	checkin <- grepl("checkin", x)
	print(x)
	readJSON(trainingPath, x, parallel=TRUE, checkin=checkin)
})
