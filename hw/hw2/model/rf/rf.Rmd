...
Just fit one big random forest (5000 trees)
```{r models}

## Fit random forests
rf <- foreach(ntree = rep(250, cores), .combine = combine, .packages = "randomForest") %dopar% randomForest(x=trainx, y=trainy, data=trainC, xtest=valx, ytest=valy, importance=TRUE, keep.forest=TRUE, ntree=ntree)
